													...
													if (plugin.arenas.get(arenaName).getTotal() < num){
														if (!plugin.arenas.get(arenaName).isResetting()){
															...
															while(list2.hasNext()){
																...
																names.sendMessage(ChatColor.DARK_PURPLE + " " + plugin.arenas.get(arenaName).getMinPlayers() + " players needed to start game. ");
															}
															if (plugin.arenas.get(plugin.playerarena.get(player)).getTotal() == plugin.arenas.get(arenaName).getMinPlayers()){
																plugin.arenas.get(arenaName).setCounter(120);
